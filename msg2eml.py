...
def load(filename_or_stream):
    if isinstance(filename_or_stream, str):
        ...
        with open(filename_or_stream, "rb") as file:
            return _load_from_stream(file)
    else:
        # If it's not a string, assume it's a file-like object
        return _load_from_stream(filename_or_stream)


def _load_from_stream(stream):
    with compoundfiles.CompoundFileReader(stream) as doc:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) <= 1:
        print(_load_from_stream(sys.stdin.buffer), file=sys.stdout)
